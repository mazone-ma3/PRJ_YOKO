...
const int SCREEN_HEIGHT = (192*2);
const float COUNT1S = 60.0f;
...
//        opt.angle += 0.08f * rate;   // 回転速度
        ...
        // 滑らかに追従
        opt.x += ((playerX + 16) - opt.x) / 4 * rate;
        opt.y += ((playerY + opt.offset_y) - opt.y) / 4 * rate;
//		float t = 1.0f - pow(1.0f - 0.25f, rate);
...
void ShooterGame::UpdateDrawFrame(void)
{
