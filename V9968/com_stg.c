...
//short
long sx,sy;
short ex,ey,ph_x,ph_y,ph_w,ph_h;
//long
short  dx,dy;
...
long score = 0, hiscore = 5000, combo = 0;
...
void update(void)
{
	...
	for(e_idx = 0; e_idx < MAX_ENEMIES; ++e_idx) {
		...
		if ((!enemies_count_flag[e_idx] && (18 <= enemies_count[e_idx]) && (enemies_count[e_idx] <= 40))){
			...
			sx = (long)player_x + 8 - (long)enemies_x[e_idx];
			sy = (long)player_y + 8 - (long)enemies_y[e_idx];
...
//			if (dist < 12) dist = 12;
	...
	// 符号を完全に分離して計算
//	if (deltax > 0) {
				dx = (signed short)((sx * enemy_bullet_speed * DIV) / dist);
//	} else if (deltax < 0) {
//		dx = -(signed short)(((-deltax) * 5 * DIV) / dist);
//	}

//	if (deltay > 0) {
				dy = (signed short)((sy * enemy_bullet_speed * DIV) / dist);
//	} else if (deltay < 0) {
...
*/
			for(eb_idx = 0; eb_idx <  MAX_e_bullets; ++eb_idx){
				if(e_bullets_active[eb_idx] == True)
					continue;
				e_bullets_x[eb_idx] = (long)enemies_x[e_idx] * DIV + 8 * DIV;
				e_bullets_y[eb_idx] = (long)enemies_y[e_idx] * DIV + 8 * DIV;
				e_bullets_xx[eb_idx] = dx;
				e_bullets_yy[eb_idx] = dy;
				e_bullets_active[eb_idx] = True;
//				put_strings(SCREEN2, 0, 24, "TAMA1", CHRPAL_NO);
...
//			direction_factor = dx / dist;
			...
			for(eb_idx = 0; eb_idx < MAX_e_bullets; ++eb_idx){
				...
				e_bullets_yy[eb_idx] = dy;
				e_bullets_active[eb_idx] = True;
//				put_strings(SCREEN2, 0, 24, "TAMA2", CHRPAL_NO);
...
/*		if ( (short)(e_bullets_x[eb_idx] / DIV) > (width - 1) - (short)(e_bullets_xx[eb_idx] / DIV) ||	  // 右に出る予定
			(short)(e_bullets_x[eb_idx] / DIV) < - (short)(e_bullets_xx[eb_idx] / DIV) ||				// 左に出る予定
			(short)(e_bullets_y[eb_idx] / DIV) > (height) - (short)(e_bullets_yy[eb_idx] / DIV) ||	  // 下に出る予定
			(short)(e_bullets_y[eb_idx] / DIV) < - (short)(e_bullets_yy[eb_idx] / DIV) ) {				// 上に出る予定
			e_bullets_active[eb_idx] = False;
			put_strings(SCREEN2, 0, 25, "KIETA", CHRPAL_NO);
		} else {
...
*/
		...
		if( ((-10+16) >= (short)(e_bullets_x[eb_idx]/DIV)) ||
			((short)(e_bullets_x[eb_idx]/DIV) >= (width + 10 + 16)) ||
			((-10+16) >= (short)(e_bullets_y[eb_idx]/DIV)) ||
			((short)(e_bullets_y[eb_idx]/DIV) >= (height + 10 + 16))){
			e_bullets_active[eb_idx] = False;

/*		if(((((long)-10+16)*DIV) >= e_bullets_x[eb_idx]) ||
			(e_bullets_x[eb_idx] >= ((long)width + 10 + 16)*DIV) ||
			((((long)-10+16)*DIV) >= e_bullets_y[eb_idx]) ||
			(e_bullets_y[eb_idx] >= ((long)height + 10 + 16)*DIV)){
			e_bullets_active[eb_idx] = False;
*/
/*
			sx = e_bullets_x[eb_idx] / DIV;
			sy = e_bullets_y[eb_idx] / DIV;

			put_numd(sx, 6);
			put_strings(SCREEN2, 0, 24, str_temp, CHRPAL_NO);

			put_numd(sy, 6);
			put_strings(SCREEN2, 10, 24, str_temp, CHRPAL_NO);
*/
//			put_numd(MAX_e_bullets, 3);
//			put_strings(SCREEN2, 10, 25, str_temp, CHRPAL_NO);
