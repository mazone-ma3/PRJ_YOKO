...
class Particle:
	def __init__(self, x, y):
		self.x = x
		self.y = y
		self.vx = int(random.uniform(-4, 4))
		self.vy = int(random.uniform(-4, 4))
...
#		self.vy *= 1 #0.92
	...
	@micropython.native
	def draw(self,s, i):
		s.set(i,max(self.x, 0), self.y,5,1, 0)
#		if self.life > 0:
...
class Option:
	...
	@micropython.native
	def draw(self,s, i):
		s.set(i,max(self.x, 0), max(self.y, 0),10,1, 0)
...
#		c = 10 if self.timer % 8 < 4 else 9
		s.set(i,max(self.x, 0), self.y,3,1, 0)
...
#		c = 12 if self.timer % 10 < 5 else 6
		s.set(i,max(self.x, 0), self.y,8,1, 0)
...
#		c = 7 if self.timer % 6 < 3 else 12
		s.set(i,max(self.x, 0), self.y,7,1, 0)
...
#		c = 8 if self.timer % 7 < 4 else 9
		s.set(i,max(self.x, 0), self.y,9,1, 0)
...
#		pyxel.sounds[4].set("c2c2c3c2", tones="t", volumes="7", effects="n", speed=8)
		...
		self.high_score = 0 #self.load_high_score()
		self.particles = []
		self.reset()

		self.sin_table = []

		for i in range(0, 256*4, 1):
...
#		self.player_y = max(0, min(self.player_y, pyxel.height - 16))
		self.player_x = max(16, min(self.player_x, 256 - 20 + 16))
		self.player_y = max(16, min(self.player_y, 192 - 16 + 16))
...
#		for eb in self.enemy_bullets[:]:
		for eb_idx in range(len(self.enemy_bullets)-1, -1, -1):
			...
			eb[0] += eb[2]
			eb[1] += eb[3]
			if not ((-10+16)*1024 < eb[0] < (256 + 10 + 16)*1024 and (-10+16)*1024 < eb[1] < (192 + 10 + 16)*1024):
...
#		for item in self.chain_items[:]:
		for item_idx in range(len(self.chain_items)-1,-1,-1):
			item = self.chain_items[item_idx]
			item.update()
...
#				pyxel.play(1, 1)
				...
				continue
			if item.x < -20+16 or item.timer <= 0:
				self.chain_count = 0
				self.chain_items.remove(item)
...
#		for item in self.option_items[:]:
		for item_idx in range(len(self.option_items)-1,-1,-1):
			item = self.option_items[item_idx]
			item.update()
...
#		for item in self.shield_items[:]:
		for item_idx in range(len(self.shield_items)-1,-1,-1):
			item = self.shield_items[item_idx]
			item.update()
			if abs(self.player_x + 8 - item.x) < 22 and abs(self.player_y + 8 - item.y) < 22:
...
#				pyxel.play(1, 1)
		...
		for item_idx in range(len(self.bomb_items)-1,-1,-1):
			item = self.bomb_items[item_idx]
...
#				pyxel.play(1, 1)
		...
		for eb_idx in range(len(self.enemy_bullets)-1, -1, -1):
			eb = self.enemy_bullets[eb_idx]
#		for eb in self.enemy_bullets:
			if (ph_x < int(eb[0]/1024) + 4 and ph_x + ph_w > int(eb[0]/1024) and
				ph_y < int(eb[1]/1024) + 4 and ph_y + ph_h > int(eb[1]/1024)):
...
#		for p in self.particles[:]:
		for p_idx in range(len(self.particles)-1,-1,-1):
...
#		s.set(i,spx[i]+16,spy[i]+16,sppat[i],sppri[i], 0)
		s.set(self.s_count,self.player_x,self.player_y,1,1, 0)
		self.s_count += 1

		if self.shield_active:
			s.set(self.s_count,self.player_x,self.player_y,6,1, 0)
			self.s_count += 1
...
#		for b in self.bullets:
		for b_idx in range(len(self.bullets)-1, -1, -1):
			b = self.bullets[b_idx]
			s.set(self.s_count,b[0], b[1],4,1, 0)
			self.s_count += 1
...
#			else: col = 11 if e[5] == 3 else (9 if e[5] == 2 else 4)
			s.set(self.s_count,max(e[0], 0), max(e[1], 0),2,1, 0)
			self.s_count += 1
...
#			pyxel.rect(e[0] + 4, int(e[1]) + 4, 8, 8, 7)
		...
		for eb_idx in range(len(self.enemy_bullets)-1, -1, -1):
			eb = self.enemy_bullets[eb_idx]
#		for eb in self.enemy_bullets:
			s.set(self.s_count,int(max(eb[0]/1024, 0)), int(max(eb[1]/1024, 0)),0,1, 0)
			self.s_count += 1
#			pyxel.rect(eb[0], eb[1], 5, 5, 8)

		for opt in self.options:
			opt.draw(s, self.s_count)
			self.s_count += 1

		for item in self.chain_items:
			item.draw(s, self.s_count)
			self.s_count += 1
		...
		for p in self.particles:
			p.draw(s, self.s_count)
			self.s_count += 1

#		print(i)

		if self.s_count < self.old_i :
			for idx in range(self.s_count, self.old_i): #self.old_i - i): #
				s.set(idx) #, 256, 256, 0, 0 , 0)
#				i = i + 1

		self.old_i = self.s_count
...
#s.defcg(5, b'\x00'*32, 0)
s.defcg(5*4, unhexlify(\
"22000000" +\
"22000000" +\
"00000000" +\
"00000000" +\
